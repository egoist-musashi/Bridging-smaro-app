...
class FileHandler(FileSystemEventHandler):
    ...
    def send_file(self, normal_file_path):
        try:
            ...
            with open(normal_file_path, 'rb') as dicom_file:
                ...
                response = requests.post(self.api_url, auth=auth, data=dicom_file, verify=False)
                logging.info(
                    f"Response from orthanc_instances api is {response.text}\n"
                )
    ...
    def create_patient(self, patient_info, patient_id):
        ...
        try:
            ...
            response = requests.post(
                "https://api.smaro.app/api/console/patient/create",
                json=patient_data,
                headers=headers,
                verify=False,
            )
    ...
    def set_write_access(self, folder_path):
        system = platform.system()
        if system == "Windows":
            # On Windows, set the folder and its contents to be writable
            ...
            logging.info(f"Write access granted to '{folder_path}' and its contents on Windows.")
        else:
            # On Unix-based systems, set permissions to allow writing (e.g., 775)
            for root, dirs, files in os.walk(folder_path):
                for dir_name in dirs:
                    os.chmod(os.path.join(root, dir_name), 0o775)
                for file_name in files:
                    os.chmod(os.path.join(root, file_name), 0o775)
            logging.info(f"Write access granted to '{folder_path}' and its contents on Unix-like system (e.g., macOS).")
